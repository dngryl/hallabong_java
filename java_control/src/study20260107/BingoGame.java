...
public class BingoGame {
	...
	public static void main(String[] args) {
		...
		int [] bingo = new int [25]; // 1부터 25의 크기
		
		for (int i=0; i<bingo.length; i++ ) { // 데이터 저장 시, 향상된 for문 (int a : bingo)을 사용하지 않는다.  
			bingo[i] = (int)Math.floor(Math.random()*100)+1;
			for (int k = 0; k<i; k++) {
				if (bingo[i] == bingo[k]) {
					i--; // 최초 for문의 i++을 상쇄시키기 위해 i--를 추가.
					break;
				}
			}
		}
		...
		while(true) {
			...
			for (int i=0; i<5; i++) {
				for(int k=0; k<5; k++) {
					...
					System.out.printf("│ %3d ", bingo[i*5+k]); // %d: 정수, %f: 실수, %c: 문자, %s: 문자열, %o: 8진수, %x: 16진수
				}
				if (i==4)
					System.out.println("│\n└─────┴─────┴─────┴─────┴─────┘");
				else
			...
			int col = 0; // 세로 방향 0의 갯수가 몇개인지 확인
			...
			for (int i =0; i<5; i++) { //줄 단위 표현은 i 변수이다.
				...
				// 해당 줄에 0이 5개라면 row, col은 5가된다 = 1빙고
				if (row==5) end++; 
				if (col==5) end++;
...
//		System.out.println(Arrays.toString(bingo)); // 간단한 출력용 확인방법 
